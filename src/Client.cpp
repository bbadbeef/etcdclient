...
pplx::task<etcd::Response> etcd::Client::send_put_request(web::http::uri_builder & uri)
{
  return Response::create(client.request(web::http::methods::PUT, uri.to_string()));
}
...
pplx::task<etcd::Response> etcd::Client::set_with_ttl(std::string const & key, std::string const & value, int ttl)
{
  web::http::uri_builder uri("/v2/keys" + key);
  if(ttl >= 0) uri.append_query("ttl="+ std::to_string(ttl));
  return send_put_request(uri, "value", value);
}

pplx::task<etcd::Response> etcd::Client::fresh_ttl(std::string const & key, int ttl)
{
  web::http::uri_builder uri("/v2/keys" + key);
  if(ttl >= 0)
  {
    uri.append_query("ttl="+ std::to_string(ttl));
    uri.append_query("refresh=true");
  }
  return send_put_request(uri);
}
