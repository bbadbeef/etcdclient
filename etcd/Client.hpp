...
namespace etcd
{
  ...
  public:
    ...
    pplx::task<Response> set_with_ttl(std::string const & key, std::string const & value, int ttl);

    pplx::task<Response> fresh_ttl(std::string const & key, int ttl);
  ...
  protected:
    ...
    pplx::task<Response> send_put_request(web::http::uri_builder & uri);
